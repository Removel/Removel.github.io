...
记忆的存储形式常常使用到数据结构当中的某类，不同的数据结构会在不同场景下的记忆有奇效：比如openhuman的树模型、my-memory的链表模型。基于这种，我有一个猜想：比如在短期的记忆我们是否可以使用双向链表+哈希表模拟LRU缓存记忆？长期的相互关联的记忆是否可以使用无向图/有向图模拟各个记忆之间的联系？
